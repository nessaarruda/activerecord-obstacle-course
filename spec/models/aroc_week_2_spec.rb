...
# ----------------------
  ...
  it "13. groups an order's items by name" do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # grouped_items = Item.joins(:order_items).where("order_id = #{@order_3.id}").order(:name) => this works
  ...
  it '15. gets all item names associated with all orders' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # names = Item.left_outer_joins(:order_items).where.not(order_items: {order_id: nil}).pluck(:name)
